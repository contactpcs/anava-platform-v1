...
from app.dependencies import require_clinical_assistant
...
async def ca_dashboard(request: Request, current_user: dict = Depends(require_clinical_assistant)):
    admin = get_supabase_admin()
    clinic_id = current_user.get("clinic_id")

    pq = admin.table("patients").select("id").eq("approval_status", "approved")
    if clinic_id:
...
async def list_patients(
    ...
    current_user: dict = Depends(require_clinical_assistant),
):
    admin = get_supabase_admin()
    clinic_id = current_user.get("clinic_id")
...
async def grant_assessment(
    ...
    # CA uses the patient's assigned doctor
