...
import { invalidateCAPatients, fetchCAPatient } from "@/store/slices/clinicalAssistantSlice";
...
function mapAnswerType(raw: string): ScaleQuestion["type"] {
  switch (raw) {
    case "radio": case "likert": case "checkbox": case "multiple_choice": return "likert";
    case "slider": case "vas": case "nrs": return "vas";
    case "number": case "integer": case "numeric": return "numeric";
...
function toPrsScaleQuestion(q: PrsAssessmentQuestion): ScaleQuestion {
  ...
  const options: QuestionOption[] | undefined = q.options?.length
    ...
    : undefined;
  return { index: q.question_index, label: q.question_text, type, required: q.is_required ?? true, options, min: q.min_value ?? undefined, max: q.max_value ?? undefined };
}

export default function CAOnBehalfAssessmentPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const loadedScales: LoadedScale[] = result.scales.map((scale) => toLoadedScale(scale, result.instance_id));
        setScales(loadedScales);

        const alreadyCompleted = new Set(result.scales.filter((s) => s.is_completed).map((s) => s.scale_id));
          ...
          try {
            ...
            for (const scale of loadedScales) {
              ...
              scale.question_ids.forEach((qid, idx) => {
                ...
                if (entry) scaleMap[String(idx)] = entry.response_value !== null && entry.response_value !== undefined ? entry.response_value : entry.given_response;
              });
              if (Object.keys(scaleMap).length > 0) restoredResponses[scale.scale_id] = scaleMap;
            }
            setResponses(restoredResponses);
            const firstIncompleteIdx = loadedScales.findIndex((s) => !alreadyCompleted.has(s.scale_id));
            if (firstIncompleteIdx >= 0) { setCurrentScaleIndex(firstIncompleteIdx); setCurrentQuestionIndex(0); }
          } catch { /* continue from beginning */ }
        }
      } catch (e: unknown) {
        const msg =
          (e as any)?.response?.data?.message ??
          (e as any)?.response?.data?.detail ??
          (e as any)?.message ?? "Failed to load assessment";
  ...
  const handleAnswer = useCallback((questionIndex: number, value: number | string) => {
    ...
    if (!scaleId) return;
    setResponses((prev) => ({ ...prev, [scaleId]: { ...(prev[scaleId] ?? {}), [String(questionIndex)]: value } }));
    const questionId = currentScale.question_ids[questionIndex];
    if (questionId) prsAssessmentService.saveResponse(currentScale.instance_id, scaleId, questionIndex, questionId, value).catch(() => {});
  }, [currentScale]);
  ...
  const handlePrev = () => {
    if (!isFirstScale) { setCurrentScaleIndex((i) => i - 1); setCurrentQuestionIndex(0); }
  };

  const handleSkipSection = () => {
    setResponses((prev) => { const next = { ...prev }; delete next[currentScale.scale_id]; return next; });
    if (isLastScale) { handleSubmitScale(); }
    else { setCurrentScaleIndex((i) => i + 1); setCurrentQuestionIndex(0); }
  };
  ...
  const handleSubmitScale = async () => {
    ...
    try {
      ...
      if (isLastScale) {
        ...
        await Promise.all(skipped.map((s) => prsAssessmentService.submitAssessment(s.instance_id, s.scale_id, {})));
        dispatch(invalidatePatientPermissions(patientId));
        dispatch(invalidatePatientScores(patientId));
        dispatch(invalidateCAPatients());
        await Promise.all([
          dispatch(fetchPatientPermissions(patientId)),
          dispatch(fetchPatientScoresSummary(patientId)),
          dispatch(fetchCAPatient(patientId)),
        ]);
        router.push(`/clinical-assistant/patients/${patientId}?section=prs`);
        router.refresh();
      } else {
      ...
      const msg = (e as any)?.response?.data?.message ?? (e as any)?.response?.data?.detail ?? (e as any)?.message ?? "Failed to submit";
      alert(String(msg));
  ...
  return (
    <AssessmentUI
      ...
      onNavigateScale={(idx) => { setCurrentScaleIndex(idx); setCurrentQuestionIndex(0); }}
