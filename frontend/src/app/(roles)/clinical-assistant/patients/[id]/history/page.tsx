...
export default function CAPatientHistoryPage() {
  ...
  useEffect(() => {
    Promise.all([
      apiClient.get(ENDPOINTS.CLINICAL_ASSISTANT.PATIENT(id))
        .then((r) => setPatientData(r.data?.data ?? null))
        .catch(() => setPatientErr("Failed to load patient profile")),

      apiClient.get(ENDPOINTS.APPOINTMENTS.LIST, { params: { patient_id: id, limit: 100 } })
  ...
  const handleDrill = useCallback(async (instanceId: string) => {
    ...
    try {
      const { data } = await apiClient.get(ENDPOINTS.CLINICAL_ASSISTANT.PATIENT_RESULT(id, instanceId));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-100 to-neutral-50">
      <div className="bg-white border-b border-neutral-200 px-8 py-4">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-neutral-700 hover:text-neutral-900 transition-colors text-sm font-medium">
      ...
      <div className="px-8 py-8 space-y-6 max-w-6xl mx-auto">
        {!patient && !patientErr ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-7">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {patient?.gender && <span className="text-sm text-neutral-600 capitalize">· {patient.gender}</span>}
                    {patient?.mrn && <span className="px-2 py-0.5 bg-neutral-100 text-neutral-600 text-xs rounded">MRN: {patient.mrn}</span>}
                    {patient?.approval_status && (
                      <span className={`px-2 py-0.5 text-xs rounded font-medium ${patient.approval_status === "approved" ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}>
              ...
              <Link href={`/clinical-assistant/patients/${id}`}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {TABS.map(({ id: tabId, label, Icon }) => (
            <button key={tabId} onClick={() => setTab(tabId)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === tabId ? "bg-neutral-900 text-white" : "bg-white text-neutral-700 border border-neutral-200 hover:border-neutral-300"}`}>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          {tab === "appointments" && (
            <div>
              <div className="px-6 py-4 border-b border-neutral-100 flex items-center gap-3 flex-wrap">
                ...
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as AppointmentStatus | "")}
                ...
                {(filterDateFrom || filterDateTo || filterStatus) && (
                  <button onClick={() => { setFilterDateFrom(""); setFilterDateTo(""); setFilterStatus(""); }} className="text-xs text-red-500 hover:text-red-700 transition-colors">Clear</button>
                )}
                <span className="ml-auto text-xs text-neutral-400">{filteredAppts.length} record{filteredAppts.length !== 1 ? "s" : ""}</span>
              </div>
          ...
          {tab === "prs" && (
            <div className="divide-y divide-neutral-100">
              {prsLoading ? <SectionSkeleton /> : prsErr ? <SectionError msg={prsErr} /> : prsScores.length === 0 ? (
                ...
                return (
                  <div key={instanceId}>
                    <button onClick={() => handleDrill(instanceId)}
                      ...
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-neutral-900 truncate">{inst.disease_name || inst.disease_id || "Assessment"}</p>
                          <p className="text-xs text-neutral-500 mt-0.5">Completed {formatDate(inst.completed_at ?? inst.time_stamp)}</p>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          {severityLbl && <span className={`text-xs font-semibold px-2 py-0.5 rounded ${severityBg(severityLevel)}`}>{severityLbl}</span>}
                        ...
                        {loading ? (
                          ...
                          <div className="pt-4 space-y-4">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {drill.disease_result.percentage != null && (
                                  <div className="bg-white rounded-lg p-3 border border-neutral-200">
                                    ...
                                    <p className="text-xl font-bold text-neutral-900">{drill.disease_result.percentage.toFixed(0)}%</p>
                                  </div>
                                )}
                                {drill.disease_result.severity_label && (
                                  <div className="bg-white rounded-lg p-3 border border-neutral-200">
                                    <p className="text-xs text-neutral-500 mb-0.5">Severity</p>
                                    <p className={`text-sm font-semibold ${severityBg(drill.disease_result.severity_level).replace(/bg-\S+ /, "")}`}>{drill.disease_result.severity_label}</p>
                                  </div>
                                )}
                              </div>
                            )}
                            {drill.scale_results?.length > 0 && (
                              <div>
                                <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide mb-2">Scale Breakdown</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                  {drill.scale_results.map((sr) => (
                                    <div key={sr.scale_result_id ?? sr.scale_id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-neutral-200">
                                      <span className="text-sm text-neutral-700 truncate">{sr.scale_name ?? sr.scale_code ?? sr.scale_id}</span>
                                      <div className="flex items-center gap-2 flex-shrink-0">
                                        {sr.calculated_value != null && (
                                          <span className="text-sm font-semibold text-neutral-900">
                                            {sr.calculated_value}{sr.max_possible != null && <span className="text-xs font-normal text-neutral-400">/{sr.max_possible}</span>}
                                          </span>
                                        )}
                                        {sr.severity_label && <span className={`text-xs px-1.5 py-0.5 rounded-full ${severityBg(sr.severity_level)}`}>{sr.severity_label}</span>}
                                      </div>
                                    </div>
                                  ))}
                                </div>
          ...
          {tab === "anamnesis" && (
            <div className="p-6">
              {anamnesisLoading ? <SectionSkeleton /> : anamnesisErr ? <SectionError msg={anamnesisErr} /> : !anamnesis ? (
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">
                      Status: <span className={`font-medium ${anamnesis.status === "completed" ? "text-green-700" : "text-amber-700"}`}>{statusLabel(anamnesis.status)}</span>
          ...
          {tab === "permissions" && (
            <div>
              {!patientData ? <SectionSkeleton /> : (patientData.permissions ?? []).length === 0 ? (
                    ...
                    <tbody className="divide-y divide-neutral-100">
                      {(patientData.permissions ?? []).map((p) => (
                        <tr key={p.permission_id} className="hover:bg-neutral-50 transition-colors">
                          <td className="px-5 py-3 text-sm font-medium text-neutral-900">{p.disease_name || p.disease_id}</td>
                          <td className="px-5 py-3">
                          ...
                          <td className="px-5 py-3 text-sm text-neutral-600 whitespace-nowrap">{formatDate(p.granted_at)}</td>
                          <td className="px-5 py-3 text-sm text-neutral-600 whitespace-nowrap">{formatDate(p.completed_at ?? p.expires_at)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
