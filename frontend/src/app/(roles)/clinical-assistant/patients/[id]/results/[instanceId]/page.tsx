...
import { useCAPatientResult } from "@/lib/hooks";
...
export default function CAPatientResultDetailPage() {
  const { id: patientId, instanceId } = useParams<{ id: string; instanceId: string }>();
  const router = useRouter();
  const detail = useCAPatientResult(patientId, instanceId);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f7f6f2] to-[#f4f0ef]">
      <div className="bg-white border-b border-neutral-200">
        <div className="max-w-full px-8 py-4 flex items-center justify-between">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-neutral-600 hover:text-neutral-900 transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-8 py-8 space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">{instance.disease_name ?? "Assessment Results"}</h1>
          <div className="flex items-center gap-4 mt-1 text-sm text-neutral-500">
            {instance.completed_at && (
              <span>Completed {new Date(instance.completed_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</span>
            )}
            ...
            <CardContent className="flex items-center justify-between gap-6">
              <div>
                ...
                <p className="text-4xl font-bold text-neutral-900">
                  {overallResult.disease_score != null ? overallResult.disease_score.toFixed(0) : "—"}
        ...
        {scale_results.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-neutral-500 uppercase tracking-wide mb-3">Scale-by-Scale Breakdown</h2>
                ...
                <Card key={sr.scale_result_id ?? sr.scale_id}>
                  <CardContent className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-sm font-semibold text-neutral-900">{sr.scale_name ?? sr.scale_code ?? sr.scale_id}</p>
                        {sr.scale_code && sr.scale_name && <p className="text-xs text-neutral-400">{sr.scale_code}</p>}
                      </div>
                      {sr.severity_label && (
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-bold text-neutral-900">{sr.calculated_value ?? "—"}</span>
                      {sr.max_possible != null && <span className="text-sm text-neutral-400">/ {sr.max_possible}</span>}
                      {sr.percentage != null && <span className="text-xs text-neutral-400 ml-1">({sr.percentage.toFixed(0)}%)</span>}
                    </div>
                        ...
                        {Object.entries(sr.subscale_scores).map(([key, sub]: [string, unknown]) => {
                          ...
                          return (
                            <div key={key} className="flex justify-between text-xs">
                              ...
                              <span className="font-medium text-neutral-700">{s.score as number}/{s.max_score as number}</span>
                            </div>
                          );
                        })}
                      </div>
