...
import {
  ChevronRight, ChevronLeft, Plus, HelpCircle, Bell, Check, Lock,
  PlayCircle, BarChart2, FileText, User,
} from "lucide-react";
...
export default function CAPatientDetailPage() {
  ...
  const patient = useCAPatient(id);
  const { patients: patientList } = useCAPatients();
  const assessments = useCAPatientPermissions(id);
  const { instances: scoreInstances, total: totalAssessments } = usePatientScoresSummary(id);
  const { record: anamnesisRecord } = usePatientAnamnesis(id);
  ...
  const [eegRefreshKey] = useState(0);

  useEffect(() => {
    if (selectedSection !== "notes") return;
    setNotesLoading(true);
    apiClient
      .get(ENDPOINTS.DOCTOR_NOTES.FOR_PATIENT(id))
      .then(({ data }) => setDoctorNotes(Array.isArray(data.data) ? data.data : data.data ? [data.data] : []))
    ...
    <div className="min-h-screen bg-gradient-to-b from-neutral-100 to-neutral-50">
      ...
      <div className="bg-white border-b border-neutral-200 px-4 sm:px-8 py-3">
        <div className="flex items-center justify-between gap-3">
          <button
            onClick={() => router.push("/clinical-assistant/patients")}
          ...
          <div className="flex items-center gap-2 flex-1 justify-end min-w-0">
            <div className="relative flex-1 max-w-xs bg-white border border-neutral-200 rounded-full items-center px-4 py-2 shadow-sm hidden sm:flex">
              <input
                type="text"
          ...
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                {patient?.mrn && <p className="text-sm text-neutral-600">({patient.mrn})</p>}
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <button
                onClick={() => prevPatient && router.push(`/clinical-assistant/patients/${prevPatient.id}`)}
              ...
              <button
                onClick={() => nextPatient && router.push(`/clinical-assistant/patients/${nextPatient.id}`)}
          ...
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            {nextAssessment ? (
              <>
                ...
                <button
                  onClick={() => router.push(`/clinical-assistant/patients/${id}/assessment/${nextAssessment.permission_id}`)}
                ...
                {buildSections(anamnesisRecord?.status ?? null).map((section) => {
                  if (section.id === "medical-history") {
                    return (
                      <Link
                        key="medical-history"
                        href={`/clinical-assistant/patients/${id}/history`}
            ...
            <div className="flex-1 bg-white rounded-lg shadow-md p-4 sm:p-8 overflow-y-auto">
              {selectedSection === "anamnesis" ? (
                ...
                <div className="space-y-5">
                  ...
                  <EEGReportList patientId={id} canDelete={false} refreshTrigger={eegRefreshKey} />
                </div>
              ) : selectedSection === "notes" ? (
                <div className="space-y-4">
                  <div>
                    <h2 className="text-2xl font-bold text-neutral-900 mb-1">Doctor&apos;s Notes</h2>
                    <p className="text-neutral-600 text-sm">Clinical notes written by the treating doctors for this patient.</p>
                  </div>
                  {notesLoading ? (
                    <div className="py-10 text-center text-sm text-neutral-400">Loading notes...</div>
                    ...
                    <div className="flex flex-col items-center justify-center py-12 border border-dashed border-neutral-200 rounded-lg">
                      ...
                      <p className="text-neutral-500 text-sm">No doctor notes yet</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {doctorNotes.map((note) => (
                        <div key={note.id} className="border border-neutral-200 rounded-lg p-5">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-neutral-900">
                                {note.profiles?.full_name ?? "Doctor"}
                              </p>
                              <p className="text-xs text-neutral-400">{formatDate(note.updated_at)}</p>
                            </div>
                          </div>
                          <p className="text-sm text-neutral-700 whitespace-pre-wrap leading-relaxed">
                            {note.note_text || <span className="text-neutral-400 italic">No content</span>}
                          </p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {completedAssessments.map((a) => {
                        const result = scoreInstances.find(
                          (s: AssessmentInstance) => s.instance_id === a.instance_id || s.disease_id === a.disease_id
                        );
                        return (
                          <div key={a.permission_id} className="border border-neutral-200 rounded-lg p-4 space-y-4">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                ...
                                {result.percentage != null && (
                                  <div className="bg-neutral-50 rounded-lg p-3">
                                    ...
                                    <p className="text-xl font-bold text-neutral-900">{result.percentage.toFixed(0)}%</p>
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            {result?.scale_summaries && result.scale_summaries.length > 0 && (
                              <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {result.scale_summaries.map((scale: any, i: number) => (
                            ...
                            <div className="flex items-center gap-3">
                              {a.instance_id ? (
                                <Link href={`/clinical-assistant/patients/${id}/results?instance_id=${a.instance_id}`}>
                    ...
                    <div className="space-y-4">
                      ...
                      {pendingAssessments.map((a) => (
                        <div key={a.permission_id} className="border border-neutral-200 rounded-lg p-4">
                          ...
                          <Link href={`/clinical-assistant/patients/${id}/assessment/${a.permission_id}`}>
