...
import { useCAPatients } from "@/lib/hooks";
...
export default function CAPatientsPage() {
  const { patients, isLoading } = useCAPatients();
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">Clinic Patients</h1>
            ...
            {filtered.map((p) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-neutral-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <button
                      onClick={() => router.push(`/clinical-assistant/patients/${p.id}`)}
