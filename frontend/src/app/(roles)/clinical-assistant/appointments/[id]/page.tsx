...
function CancelDialog({ onConfirm, onClose, busy }: { onConfirm: (reason: string) => void; onClose: () => void; busy: boolean }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex items-start justify-between px-5 py-4 border-b border-neutral-100">
          ...
          <button onClick={onClose} className="p-1 text-neutral-400 hover:text-neutral-600 rounded-lg hover:bg-neutral-100"><X className="w-4 h-4" /></button>
        </div>
        <div className="px-5 py-4 space-y-3">
          <p className="text-sm text-neutral-600">Please provide a reason for cancellation.</p>
          <textarea rows={3} placeholder="Cancellation reason…" value={reason} onChange={(e) => setReason(e.target.value)}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-neutral-100">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-100 rounded-lg transition-colors">Back</button>
          <button disabled={reason.trim().length < 3 || busy} onClick={() => onConfirm(reason.trim())}
...
function RescheduleModal({ doctorId, onConfirm, onClose, busy }: { doctorId: string; onConfirm: (date: string, time: string, reason?: string) => void; onClose: () => void; busy: boolean }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            {loadingSlots ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {slots.map((s) => (
                  <button key={s.start_time} onClick={() => setTime(s.start_time)}
                    className="text-xs px-2 py-1.5 rounded-lg font-medium transition-colors"
                    style={time === s.start_time ? { background: BRAND, color: "#fff" } : { background: "#f0f9ff", color: "#0369a1", border: "1px solid #bae6fd" }}>
                    {fmt12(s.start_time)}
                  </button>
                ))}
              </div>
          ...
          <div>
            <label className="text-xs font-medium text-neutral-600 block mb-1">Reason <span className="text-neutral-400 font-normal">(optional)</span></label>
            <input type="text" placeholder="e.g. Doctor unavailable" value={reason} onChange={(e) => setReason(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-300" />
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-neutral-100">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-100 rounded-lg transition-colors">Back</button>
          <button disabled={!date || !time || busy} onClick={() => onConfirm(date, time, reason || undefined)}
            className="px-4 py-2 text-sm font-medium text-white rounded-lg transition-opacity hover:opacity-90 disabled:opacity-50" style={{ background: BRAND }}>
...
function ActionBtn({ icon: Icon, label, color, busy, onClick }: { icon: React.ElementType; label: string; color: BtnColor; busy: boolean; onClick: () => void }) {
...
export default function CAAppointmentDetailPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try { const hist = await appointmentsService.getHistory(id); setHistory(hist); }
    catch { setHistory([]); }
  ...
  const run = useCallback(async (fn: () => Promise<any>, label: string) => {
    ...
    try { await fn(); }
    catch (e: any) { setActionError(e?.response?.data?.detail ?? e?.message ?? `${label} failed`); }
  ...
  const saveNotes = async () => {
    ...
    setBusy(true);
    try { await appointmentsService.update(id, { notes: notesVal }); setEditingNotes(false); }
    catch { /* ignore */ }
    finally { setBusy(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <Link href="/clinical-assistant/appointments"
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-neutral-200 p-5">
            <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
              <div>
                <h1 className="text-xl font-bold text-neutral-900">{appointment.patient_name ?? "Patient"}</h1>
                <p className="text-sm text-neutral-500 mt-0.5 capitalize">{(appointment.appointment_type ?? "consultation").replace("_", " ")}</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2.5 text-sm text-neutral-700">
                ...
                <Link href={`/clinical-assistant/patients/${appointment.patient_id}`}
                  className="hover:underline font-medium" style={{ color: "#00A1E4" }}>
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              {!editingNotes ? (
                <button onClick={() => setEditingNotes(true)} className="flex items-center gap-1 text-xs font-medium hover:underline" style={{ color: "#00A1E4" }}>
                  <Pencil className="w-3 h-3" /> Edit
                </button>
              ) : (
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingNotes(false); setNotesVal(appointment.notes ?? ""); }} className="text-xs text-neutral-500 hover:text-neutral-700">Cancel</button>
                  <button onClick={saveNotes} disabled={busy} className="flex items-center gap-1 text-xs font-medium text-white px-2.5 py-1 rounded-lg disabled:opacity-50" style={{ background: BRAND }}>
                ...
                {history.map((h: any, i: number) => (
                  <div key={h.history_id ?? i} className="flex items-start gap-3 text-xs">
                    ...
                    <div>
                      ...
                      {h.changed_by_role && <span className="text-neutral-400 ml-1">by {h.changed_by_role.replace("_", " ")}</span>}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-neutral-200 p-5">
            ...
            {actionError && (
              <div className="mb-3 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-600">{actionError}</div>
            )}
            <div className="space-y-2">
              {canConfirm    && <ActionBtn icon={CheckCircle2} label="Confirm"           color="green"  busy={busy} onClick={() => run(confirm, "Confirm")} />}
              {canCheckIn    && <ActionBtn icon={UserCheck}    label="Check In Patient"  color="blue"   busy={busy} onClick={() => run(checkIn, "Check-in")} />}
              {canStart      && <ActionBtn icon={Play}         label="Start Consultation" color="brand" busy={busy} onClick={() => run(start, "Start")} />}
              {canComplete   && <ActionBtn icon={CheckSquare}  label="Mark Complete"     color="green"  busy={busy} onClick={() => run(complete, "Complete")} />}
              {canReschedule && <ActionBtn icon={RotateCcw}    label="Reschedule"        color="purple" busy={busy} onClick={() => setShowReschedule(true)} />}
              {canNoShow     && <ActionBtn icon={AlertOctagon} label="Mark No-Show"      color="orange" busy={busy} onClick={() => run(noShow, "No-show")} />}
              {canCancel     && <ActionBtn icon={XCircle}      label="Cancel"            color="red"    busy={busy} onClick={() => setShowCancel(true)} />}
              {["completed", "cancelled", "no_show", "rescheduled"].includes(status) && (
                <div className="w-full text-center text-xs py-2 rounded-lg font-medium" style={{ background: cfg.bg, color: cfg.text }}>{cfg.label}</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 p-5">
            ...
            <dl className="space-y-2 text-xs">
              ...
              <MetaRow label="Type"     value={(appointment.appointment_type ?? "consultation").replace("_", " ")} capitalize />
              <MetaRow label="Booked by" value={(appointment.booked_by_role ?? "").replace("_", " ")} capitalize />
              <MetaRow label="Created"  value={new Date(appointment.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} />
            </dl>
          </div>
        </div>
      </div>
      ...
      {showCancel && (
        <CancelDialog busy={busy} onClose={() => setShowCancel(false)}
          onConfirm={(reason) => { setShowCancel(false); run(() => cancel(reason), "Cancel"); }} />
      )}
      {showReschedule && (
        <RescheduleModal doctorId={docId} busy={busy} onClose={() => setShowReschedule(false)}
          onConfirm={(date, time, reason) => { setShowReschedule(false); run(() => reschedule(date, time, reason), "Reschedule"); }} />
      )}
    </div>
  );
}
