...
export default function CADashboard() {
  ...
  const fetchRange = useMemo(() => {
    if (view === "Week") return { from: weekMonday, to: addDays(weekMonday, 6) };
    if (view === "Day")  return { from: dayDate, to: dayDate };
  ...
  // CA views clinic doctor schedules — fetch first available doctor's slots
  const fetchSlots = useCallback(async (from: Date, to: Date) => {
    try {
      const { data: staffData } = await apiClient.get(ENDPOINTS.SCHEDULE.MY, {
        params: { from_date: toDateStr(from), to_date: toDateStr(to) },
      }).catch(() => ({ data: { data: [] } }));
      setSlots(staffData?.data ?? []);
    } catch { setSlots([]); }
  }, []);
  ...
  const calLabel = useMemo(() => {
    ...
    if (view === "Day") {
      return dayDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" });
    }
  ...
  const displayName  = user?.first_name || "Clinical Assistant";
  const todayDisplay = today.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" });
      ...
      >
        ...
        {ghost?.colIdx === colIdx && (
          <div
            ...
            style={{ top: ghost.top, height: Math.max(ghost.height, 24), backgroundColor: "rgba(34,197,94,0.12)", border: "1.5px dashed #4ade80" }}
          />
        )}
        {dayAppts.map((appt) => {
          ...
          return (
            <div
              ...
              style={{ top, height: Math.min(height, gridHeight - top), backgroundColor: sty.bg, borderLeft: `3px solid ${sty.border}`, color: sty.text }}
              onClick={(e) => { e.stopPropagation(); router.push(`/clinical-assistant/appointments/${appt.appointment_id}`); }}
            >
              <div className="text-[10px] font-semibold leading-tight truncate">
                {fmt12(appt.start_time).replace(" AM","").replace(" PM","")}–{fmt12(appt.end_time).replace(" AM","").replace(" PM","")}
              </div>
              {height >= 36 && (
                <div className="text-[10px] truncate mt-0.5 opacity-80">{appt.patient_name || "Patient"}</div>
              )}
            </div>
          );
        })}
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">Welcome back, {displayName}!</h1>
          <p className="text-sm text-neutral-500 mt-0.5">Here&apos;s what&apos;s happening in your clinic today.</p>
        </div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-[1fr_280px] mb-6">
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-card overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100">
            ...
            <Link href="/clinical-assistant/appointments" className="text-sm font-medium hover:underline" style={{ color: "#00A1E4" }}>
              View all
            </Link>
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <Link href={`/clinical-assistant/patients/${appt.patient_id}`} className="flex-shrink-0">
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    <Link href={`/clinical-assistant/appointments/${appt.appointment_id}`}>
                      <button
        ...
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-card p-6">
          ...
          <div className="space-y-3">
            <button className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-white font-medium text-sm transition-opacity hover:opacity-90" style={{ background: BRAND }}>
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-card mb-6 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5">
              <button onClick={goToday} className="px-3 py-1.5 text-sm font-medium text-neutral-700 bg-white border border-neutral-200 rounded-lg hover:bg-neutral-50 transition-colors">Today</button>
              <button onClick={goPrev} className="p-1.5 text-neutral-500 bg-white border border-neutral-200 rounded-lg hover:bg-neutral-50 transition-colors"><ChevronLeft className="w-4 h-4" /></button>
              <button onClick={goNext} className="p-1.5 text-neutral-500 bg-white border border-neutral-200 rounded-lg hover:bg-neutral-50 transition-colors"><ChevronRight className="w-4 h-4" /></button>
            ...
            <div className="flex items-center rounded-lg border border-neutral-200 overflow-hidden text-sm font-medium">
              {(["Week", "Day", "Month"] as CalView[]).map((v) => (
                <button key={v} onClick={() => setView(v)} className="px-3 py-1.5 transition-colors"
                  style={view === v ? { background: BRAND, color: "#fff" } : { background: "#fff", color: "#525252" }}>
                  {v}
                </button>
              ))}
            </div>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: 700 }}>
              <div className="grid border-b border-neutral-100" style={{ gridTemplateColumns: "52px repeat(7, 1fr)" }}>
                <div />
                {getWeekDates(weekMonday).map((d, i) => {
                  ...
                  return (
                    <div key={i} className="text-center py-3 px-1 border-l border-neutral-100">
                      ...
                      <div className="inline-flex items-center justify-center w-8 h-8 mx-auto rounded-full text-sm font-bold"
                  ...
                  <div className="relative">
                    {hours.map((h) => (
                      <div key={h} className="absolute left-0 right-0 flex items-start justify-end pr-2"
                        ...
                        <span className="text-[10px] text-neutral-400 font-medium mt-1">{String(h).padStart(2, "0")}:00</span>
                      </div>
                    ))}
                  </div>
                  {getWeekDates(weekMonday).map((d, colIdx) => renderTimeColumn(toDateStr(d), colIdx))}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: 560 }}>
              <div className="grid border-b border-neutral-100" style={{ gridTemplateColumns: "repeat(7, 1fr)" }}>
                {DAY_LABELS.map((l) => (
                  <div key={l} className="text-center py-3 text-[11px] font-semibold text-neutral-500 uppercase tracking-wide border-r border-neutral-100 last:border-r-0">{l}</div>
                ))}
              </div>
              {getMonthGrid(monthDate.getFullYear(), monthDate.getMonth()).map((week, wi) => (
                <div key={wi} className="grid border-b border-neutral-100 last:border-b-0" style={{ gridTemplateColumns: "repeat(7, 1fr)" }}>
                  {week.map((d, di) => {
                    const ds = toDateStr(d);
                    const isToday    = ds === todayStr;
                    ...
                    return (
                      <div key={di}
                        className={`border-r border-neutral-100 last:border-r-0 p-2 min-h-[80px] transition-colors ${isCurMonth ? "cursor-pointer hover:bg-neutral-50" : "bg-neutral-50/50"}`}
                        onClick={() => { if (!isCurMonth) return; setDayDate(d); setView("Day"); }}
                      >
                        <div className="inline-flex items-center justify-center w-7 h-7 rounded-full text-sm font-semibold mb-1"
                          style={isToday ? { background: BRAND, color: "#fff" } : { color: isCurMonth ? "#262626" : "#d4d4d4" }}>
                          {d.getDate()}
                        </div>
                        {dayAppts.length > 0 && (
                          <div className="flex flex-wrap gap-0.5 mb-1">
                            {dayAppts.slice(0, 4).map((a) => (
                              <div key={a.appointment_id} className="w-2 h-2 rounded-full flex-shrink-0"
                                style={{ backgroundColor: STATUS_DOT[a.status] ?? "#94a3b8" }} />
                            ))}
                            {dayAppts.length > 4 && <span className="text-[9px] text-neutral-400">+{dayAppts.length - 4}</span>}
                          </div>
                        )}
                        {dayAppts.length > 0 && (
                          <div className="text-[10px] text-neutral-500 leading-tight">{dayAppts.length} appt{dayAppts.length !== 1 ? "s" : ""}</div>
                        )}
                      </div>
                    );
                  })}
        ...
        <div className="flex items-center justify-center gap-6 px-6 py-3 border-t border-neutral-100 flex-wrap">
          {[{ label: "Confirmed", color: "#22c55e" }, { label: "Pending", color: "#f59e0b" }, { label: "Cancelled", color: "#ef4444" }].map(({ label, color }) => (
      ...
      {bookingSlot && (
        <BookingModal
          slot={bookingSlot}
          doctorId={(bookingSlot as any).doctor_id ?? ""}
