...
function buildDiff(current: Record<string, string>, original: Record<string, string>): Record<string, string | number> {
...
const inputCls = "w-full rounded-lg border border-neutral-200 bg-white px-3.5 py-2.5 text-sm text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:border-sky-400 hover:border-neutral-300 transition-all";
...
export default function CAProfilePage() {
  ...
  const [notifPrefs, setNotifPrefs] = useState({ newAppts: true, revenueUpdates: false, platformUpdates: true });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiClient.get(ENDPOINTS.CLINICAL_ASSISTANT.DASHBOARD).catch(() => ({ data: { data: {} } })),
      apiClient.get(ENDPOINTS.CLINICAL_ASSISTANT.PATIENTS, { params: { limit: 100 } }).catch(() => ({ data: { data: [] } })),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const freshFilled: FormState = {
        ...
        employee_id:   (updated.employee_id   as string) ?? "",
      };
      setForm(freshFilled); originalRef.current = freshFilled;
      setProfileRaw(updated as unknown as Record<string, unknown>);
      dispatch(updateUserInStore({ first_name: updated.first_name, last_name: updated.last_name, full_name: updated.full_name, city: updated.city, gender: updated.gender, date_of_birth: updated.date_of_birth }));
  ...
  const empId    = (profileRaw?.employee_id as string) ?? (user?.id?.slice(0, 8).toUpperCase() ?? "—");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="rounded-2xl border border-neutral-200 bg-gradient-to-br from-sky-50 via-blue-50/40 to-sky-50 p-6 border-l-4" style={{ borderLeftColor: BRAND_PX }}>
        <div className="flex items-center gap-5">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-neutral-400 mt-0.5">Employee ID: {empId}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-6 pt-5 border-t border-blue-100">
          <div>
            <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-widest mb-1">Department</p>
            <p className="text-sm font-semibold text-neutral-800">{form.department || "—"}</p>
          </div>
          <div>
            <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-widest mb-1">Designation</p>
            <p className="text-sm font-semibold" style={{ color: BRAND_PX }}>{form.designation || "—"}</p>
          </div>
          <div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 overflow-hidden">
          ...
          {TABS.map(({ id, label, Icon }) => {
            ...
            return (
              <button key={id} onClick={() => setActiveTab(id)}
                className={`flex items-center gap-2 px-5 py-4 text-sm font-medium whitespace-nowrap transition-colors border-b-2 -mb-px flex-shrink-0 ${active ? "text-neutral-900 border-sky-500" : "text-neutral-500 border-transparent hover:text-neutral-700"}`}>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div>
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    ...
                    <div><label className={labelCls}>Department</label><input className={inputCls} value={form.department} placeholder="e.g. Neurology, Psychiatry…" onChange={(e) => set("department", e.target.value)} /></div>
                    <div><label className={labelCls}>Designation</label><input className={inputCls} value={form.designation} placeholder="e.g. Clinical Assistant, Senior CA…" onChange={(e) => set("designation", e.target.value)} /></div>
                    <div><label className={labelCls}>Employee ID</label><input className={inputCls} value={form.employee_id} onChange={(e) => set("employee_id", e.target.value)} /></div>
                    ...
                    <div>
                      ...
                      <select className={inputCls} value={form.language_pref} onChange={(e) => set("language_pref", e.target.value)}>
                        ...
                        {["en","hi","te","ta","mr","kn","ml"].map((l) => (<option key={l} value={l}>{l.toUpperCase()}</option>))}
                      </select>
                    </div>
                    {saveError && (
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={handleSave} disabled={isSaving}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-sm font-semibold disabled:opacity-50 hover:opacity-90 transition-opacity" style={{ background: BRAND }}>
                  ...
                  <>
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      <button onClick={() => { setSaveSuccess(false); setIsEditing(true); }}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-sm font-semibold hover:opacity-90 transition-opacity" style={{ background: BRAND }}>
              ...
              <div className="space-y-6">
                <div>
                  <h2 className="text-base font-bold text-neutral-900 mb-3">Activity Statistics</h2>
                  <div className="space-y-3">
        ...
        {activeTab === "finance" && (
          <div className="p-6">
            <h2 className="text-base font-bold text-neutral-900 mb-5">Activity &amp; Statistics</h2>
            ...
            <div className="rounded-xl border border-neutral-200 py-10 text-center text-sm text-neutral-400">
        ...
        {activeTab === "settings" && (
          <div className="p-6">
              ...
              <div className="rounded-xl border border-neutral-200 overflow-hidden">
                ...
                <div className="divide-y divide-neutral-100">
                  {([
                    ...
                    { key: "revenueUpdates",  label: "Revenue Updates",          desc: "Weekly activity summaries"           },
            ...
            <button className="mt-5 w-full py-3 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition-opacity" style={{ background: BRAND }}>
              Save Settings
            </button>
          </div>
        )}
